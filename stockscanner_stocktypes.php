...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .found-item { position: relative; background: #1e293b; border-radius: 12px; padding: 22px 16px 16px 16px; border: 1px solid #334155; display: grid; grid-template-columns: 1fr auto auto auto; gap: 16px; align-items: center; }
        .found-item:hover { border-color: #3b82f6; }
        .type-badge { position: absolute; top: -10px; left: 16px; font-size: 0.65rem; font-weight: 700; padding: 3px 10px; border-radius: 8px; text-transform: uppercase; letter-spacing: 0.04em; color: #fff; box-shadow: 0 2px 6px rgba(0,0,0,0.4); }
        .type-pre-breakout { background: #22c55e; }
        .type-stalwart { background: #2563eb; }
        .type-fast-grower { background: #db2777; }
        .type-cyclical { background: #f59e0b; }
        .type-turnaround { background: #10b981; }
        .type-asset-play { background: #8b5cf6; }
        .type-slow-grower { background: #64748b; }
...
<body>
    ...
    (function() {
        ...
        function appendFoundItem(item, animate) {
            ...
            var typeLabel = item.type || 'Pre-Breakout';
            var typeSlug = typeLabel.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');

            div.innerHTML =
                '<div class="type-badge type-' + typeSlug + '">' + typeLabel + '</div>' +
...
function runPreBreakoutSearch($progressFile) {
    ...
    $validSymbol = null; $validTicker = null; $validPrice = null; $validVolRatio = null; $validSparkline = []; $validType = null;
        ...
        while ($offset < count($candidates) && !$validSymbol) {
            ...
            foreach ($chartHandles as $symbol => $info) {
                ...
                if (!$validSymbol) {
                    ...
                    $matchType = 'Pre-Breakout';
                    if (!$result) {
                        // Same chart data, no extra call: check it against the 6 Peter Lynch stock types
                        $result = classifyStockShape($body, $maxPrice);
...
// Classifies a symbol into one of Peter Lynch's 6 stock-shape categories
// (Stalwart, Fast Grower, Cyclical, Turnaround, Asset Play, Slow Grower)
// using only the 3mo daily price/volume chart data already fetched for the
// pre-breakout check — no additional API calls. Only one type is returned
// per stock, picked by priority (most specific pattern first).
function classifyStockShape($responseBody, $maxPrice = 4.0) {
    ...
    $currentPrice = $prices[$n - 1];
    if ($currentPrice > $maxPrice || $currentPrice < 0.5) return null;

    // Break the period into chunks to read the overall shape of the move
    $chunkCount = 6;
    $chunkSize = (int)floor($n / $chunkCount);
    if ($chunkSize < 3) return null;
    $chunkAvgs = [];
    for ($i = 0; $i < $chunkCount; $i++) {
        $start = $i * $chunkSize;
        $end = ($i === $chunkCount - 1) ? $n : $start + $chunkSize;
        $slice = array_slice($prices, $start, $end - $start);
        if (empty($slice)) continue;
        $chunkAvgs[] = array_sum($slice) / count($slice);
    }
    $chunks = count($chunkAvgs);
    if ($chunks < 4) return null;

    $first = $chunkAvgs[0];
    $last = $chunkAvgs[$chunks - 1];
    $mid = $chunkAvgs[(int)floor($chunks / 2)];
    $totalReturnPct = $first > 0 ? (($last - $first) / $first) * 100 : 0;

    // Daily volatility (stdev of returns) tells us how smooth/choppy the move is
    $returns = [];
    for ($i = 1; $i < $n; $i++) {
        if ($prices[$i - 1] > 0) $returns[] = ($prices[$i] - $prices[$i - 1]) / $prices[$i - 1];
    }
    $meanRet = !empty($returns) ? array_sum($returns) / count($returns) : 0;
    $variance = 0;
    foreach ($returns as $r) $variance += pow($r - $meanRet, 2);
    $variance = !empty($returns) ? $variance / count($returns) : 0;
    $volatility = sqrt($variance) * 100;

    // Count wave-like direction changes chunk-to-chunk (for cyclicals)
    $directionChanges = 0; $prevDir = null;
    for ($i = 1; $i < $chunks; $i++) {
        $diff = $chunkAvgs[$i] - $chunkAvgs[$i - 1];
    ...
    // Drop then recovery (for turnarounds)
    $minChunk = min($chunkAvgs);
    $minIdx = array_search($minChunk, $chunkAvgs);
    $dropFromFirstPct = $first > 0 ? (($minChunk - $first) / $first) * 100 : 0;
    $recoveryFromMinPct = $minChunk > 0 ? (($last - $minChunk) / $minChunk) * 100 : 0;

    // Long flat stretch then a sudden late repricing jump (for asset plays)
    $earlyChunks = array_slice($chunkAvgs, 0, $chunks - 1);
    $earlyRangePct = 0;
    if (!empty($earlyChunks)) {
        $eMin = min($earlyChunks); $eMax = max($earlyChunks);
        $earlyRangePct = $eMin > 0 ? (($eMax - $eMin) / $eMin) * 100 : 0;
    }
    $lastJumpPct = $mid > 0 ? (($last - $mid) / $mid) * 100 : 0;

    $type = null;
    if ($earlyRangePct < 8 && $lastJumpPct > 20) {
        $type = 'Asset Play';
    } elseif ($dropFromFirstPct <= -15 && $recoveryFromMinPct >= 25 && $minIdx > 0 && $minIdx < $chunks - 1) {
        $type = 'Turnaround';
    } elseif ($directionChanges >= 3) {
        $type = 'Cyclical';
    } elseif ($totalReturnPct >= 25 && $last > $mid && $mid > $first) {
        $type = 'Fast Grower';
    } elseif ($totalReturnPct >= 6 && $totalReturnPct < 25 && $volatility < 4) {
        $type = 'Stalwart';
    } elseif ($totalReturnPct > -5 && $totalReturnPct < 8 && $volatility < 2.5) {
        $type = 'Slow Grower';
    }
...
?>
<!-----https://www.youtube.com/watch?v=-XF-gs_SXnA--->