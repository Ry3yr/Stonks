 ...
 * Primary:   TradingView Most Volatile (US stocks)
 * Secondary: NASDAQ Market Cap (Medium/Small/Micro/Nano)
 */
...
$MIN_PRICE = 0.50;
$MAX_PRICE = 50.00;
$MIN_PCT_CHANGE = 5.0;  // ← NEW: Minimum % change to even consider
$LOOKBACK_DAYS = 20;
...
function runStreamingScan() {
    ...
    $stats['tv_total'] = count($tvCandidates);
    
    sendEvent('status', [
        'message' => "Found {$stats['tv_total']} volatile stocks from TradingView, screening...", 
        'type' => 'info'
    ]);
    
    foreach ($tvCandidates as $idx => $sym) {
        $stats['tv_checked']++;
        
        if ($idx % 5 === 0) {
            sendEvent('progress', [
                'source' => 'TradingView',
                'checked' => $stats['tv_checked'],
                'total' => $stats['tv_total'],
                'signals' => $stats['tv_signals'],
                'symbol' => $sym
            ]);
        }
    ...
    sendEvent('status', [
        'message' => "TradingView done: {$stats['tv_signals']} signals found", 
        'type' => 'info'
    ]);
    ...
    foreach ($allNasdaqStocks as $cand) {
        ...
        // ── FILTER 1: Price too high ──
        if ($price > $MAX_PRICE) {
            $skipReasons['price_too_high']++;
            continue;
        }
        
        // ── FILTER 2: Price too low ──
        if ($price < $MIN_PRICE) {
            $skipReasons['price_too_low']++;
            continue;
        }
        
        // ── FILTER 3: Market cap too big ──
        if ($cap > 10_000_000_000) {
            $skipReasons['cap_too_big']++;
            continue;
        }
        
        // ── FILTER 4: % Change too low (NEW - THIS IS HUGE!) ──
        // We only care about stocks moving significantly
        if (abs($pctChange) < $MIN_PCT_CHANGE) {
            $skipReasons['pct_change_too_low']++;
            continue;
        }
        
        // ── FILTER 5: No valid data ──
        if ($price <= 0 || $cap <= 0) {
            $skipReasons['no_data']++;
            continue;
        }
    ...
    $keptPercent = $stats['nas_total'] > 0 ? round(($stats['nas_total'] / $stats['nas_original']) * 100, 1) : 0;
    
    sendEvent('status', [
        'message' => "NASDAQ: {$stats['nas_original']} → {$stats['nas_total']} candidates (skipped {$totalSkipped}) - Price: {$skipReasons['price_too_high']}, Change<{$MIN_PCT_CHANGE}%: {$skipReasons['pct_change_too_low']}, Cap: {$skipReasons['cap_too_big']}", 
        'type' => 'info'
    ]);
    ...
    foreach ($filteredCandidates as $idx => $cand) {
        ...
        usleep(100000); // Even faster since we filtered more
    }
    
    sendEvent('done', [
        'message' => "✅ Complete! Found " . ($stats['tv_signals'] + $stats['nas_signals']) . " signals. NASDAQ: {$stats['nas_checked']}/{$stats['nas_total']} checked (filtered out {$stats['nas_skipped_total']} stocks using price, cap, and % change)", 
        'stats' => $stats
    ]);
}
    ...
    foreach ($allRows as $row) {
        ...
        if ($band !== null && in_array($band, $wanted, true)) {
            $filtered[] = [
                ...
                'pctchange' => $row['pctchange'] ?? '0%',  // ← KEEP THIS!
                'band' => $band,
            ];
        }
    }
...
function screen(array $h, string $sym): ?array {
    ...
    if ($price >= $high20 * $BREAKOUT_THRESHOLD && $vol >= $avgVol * $VOLUME_SPIKE && $rsi <= $MAX_RSI_BREAKOUT) {
        return [
            ...
            'rsi' => round($rsi,1), 'spike' => round($vol/$avgVol,1),
    ...
    if ($vol >= $avgVol * $VOL_SURGE_MIN && $priceChange >= 0.03 && $rsi < 75) {
        return [
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .stat-box { background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 12px 18px; min-width: 120px; }
        ...
        th { background: #21262d; color: #58a6ff; padding: 12px 14px; text-align: right; font-weight: 600; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid #30363d; cursor: pointer; user-select: none; transition: background 0.15s; white-space: nowrap; position: sticky; top: 0; z-index: 10; }
        th:hover { background: #30363d; }
        th:first-child, td:first-child { text-align: left; }
        th .arrow { margin-left: 6px; opacity: 0.5; font-size: 10px; }
        ...
        .rsi-ok { color: #3fb950; }
        .rsi-warn { color: #d29922; }
        .rsi-hot { color: #f85149; }
        .mom-pos { color: #3fb950; }
        .mom-neg { color: #f85149; }
        .none { color: #8b949e; font-style: italic; padding: 60px 20px; text-align: center; font-size: 15px; }
        .none-sub { color: #484f58; font-size: 12px; margin-top: 8px; }
        .source-header { display: flex; gap: 12px; margin-bottom: 12px; }
        ...
        .filter-badge { background: #21262d; color: #8b949e; padding: 2px 8px; border-radius: 4px; font-size: 10px; margin-left: 4px; }
        .filter-badge.highlight { color: #58a6ff; }
    </style>
</head>
<body>
    ...
    <div class="results-container" id="resultsContainer">
        <table id="signalTable">
            <thead>
                <tr>
                    <th data-col="symbol" data-type="string">Symbol<span class="arrow"></span></th>
                    <th data-col="source" data-type="string">Source<span class="arrow"></span></th>
                    <th data-col="strategy" data-type="string">Strategy<span class="arrow"></span></th>
                    <th data-col="trigger" data-type="string">Trigger<span class="arrow"></span></th>
                    <th data-col="trigger_pct" data-type="number">Trigger %<span class="arrow"></span></th>
                    <th data-col="price" data-type="number">Price<span class="arrow"></span></th>
                    <th data-col="vol" data-type="number">Volume<span class="arrow"></span></th>
                    <th data-col="avg_vol" data-type="number">Avg Vol<span class="arrow"></span></th>
                    <th data-col="spike" data-type="number">Spike<span class="arrow"></span></th>
                    <th data-col="rsi" data-type="number">RSI<span class="arrow"></span></th>
                    <th data-col="mom3d" data-type="number">3d Mom<span class="arrow"></span></th>
                    <th data-col="band" data-type="string">Band<span class="arrow"></span></th>
                </tr>
            </thead>
            <tbody id="signalBody">
    ...
    function startScan() {
        ...
        eventSource.addEventListener('progress', function(e) {
            ...
            if (source === 'TradingView') {
                stats.tv_checked = checked;
                stats.tv_total = total;
            } else {
                stats.nas_checked = checked;
                stats.nas_total = total;
                stats.nas_skipped_total = skipped;
            }
            
            const totalItems = stats.tv_total + stats.nas_total;
            const checkedItems = stats.tv_checked + stats.nas_checked;
    ...
    function renderSignal(sig) {
        ...
        const sourceClass = sig.source === 'TradingView' ? 'source-tv' : 'source-nas';
        ...
        tr.innerHTML = `
            <td class="sym"><a href="<?= $TICKER_BASE_URL ?>${escapeHtml(sig.symbol)}" target="_blank" rel="noopener">${escapeHtml(sig.symbol)}</a></td>
            <td><span class="source-badge ${sourceClass}">${sig.source === 'TradingView' ? 'TV' : 'NAS'}</span></td>
    ...
    function updateStats() {
        ...
        const totalItems = stats.tv_total + stats.nas_total;
        const checkedItems = (stats.tv_checked || 0) + (stats.nas_checked || 0);
        const progress = totalItems > 0 ? Math.round((checkedItems / totalItems) * 100) : 0;
        document.getElementById('progressCount').textContent = progress + '%';
    }
